...
This document is the product contract for Sindook 0.7.0: what is stable,
what may change, and what is explicitly not promised. It is accurate for the
v0.7.0 release and updated on every release. Historical behavior is marked
as such.
...
newer. `go install github.com/ruddro-roy/sindook/cmd/sindook@v0.7.0` works
...
sindook version   # "sindook 0.7.0"
```

Source-tree builds report `sindook 0.7.0-dev` plus commit provenance, and
...
- Upgrades within v0.x are drop-in: format v2 is unchanged, config schema v1 is unchanged, and identities/contacts are untouched. v0.6.0 users upgrade to v0.7.0 by replacing the binary.
